...
pub(crate) use sound::{guna_of, is_jhal, is_vibhakti_protected_final, is_vowel, vrddhi_of};
...
mod tests {
    ...
    // `pub(super)` so the stage files' test modules can reach it;
    // `shatva_declines_for_every_pre_existing_junction` in `tripadi.rs` does.
