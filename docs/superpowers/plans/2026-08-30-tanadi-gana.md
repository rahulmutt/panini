...
- **8.2.77 *hali ca* / 8.2.79 *na bhakurchurām***: neither implemented; the pair is inseparable (77 without 79 derives \*kUryAt) and no covered form needs either. Recorded in the √kṛ block comment. [Superseded during execution (Task 12 review, ruling C2): 8.2.77 *hali ca* has been implemented since the divādi slice and fired on `kur`; 8.2.79 is modelled as a named guard clause inside 8.2.77 in `tripadi.rs`.]
