...
curation, √ric and √vic for a guard that still reads only `j` where the sūtra
(8.2.30 *coḥ kuḥ*) names the whole cu-varga, and √chid and √chṛd for two sūtras
the engine does not implement (6.1.73 *che ca*, 8.4.40 *stoḥ ścunā ścuḥ*).
...
`rundDAd` / `runDAd` / `rundDAt` / `runDAt`; `check` reports every derivation,
each with its own trace. A root may also admit **both** padas — √rudh derives a
full parasmaipada and a full ātmanepada paradigm, and `check` reports each
analysis with its own pada. 1.3.72's semantic condition (*kartrabhiprāye
kriyāphale*, the fruit of the action accruing to the agent) is **not**
